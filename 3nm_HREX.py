...
from alchemlyb.postprocessors.units import to_kJmol
import argparse
...
def convergence(estimator):
    ...
    df.attrs['energy_unit'] = 'kJ/mol'
    ax = plot_convergence(df, units='kJ/mol')
    ax.legend(labels=('_Error','Forward','Reverse'),loc='upper right',fontsize=18,frameon=False)
...
if estimator.lower() == 'ti' or estimator.lower() == 'all':
    ...
    ti_kJ = to_kJmol(ti.delta_f_, T=temp)
    dti_kJ = to_kJmol(ti.d_delta_f_, T=temp)
    ti_all = pd.concat([ti_kJ, dti_kJ])
    ti_all.to_csv('TI.csv')

    #Get estimates from df
    TI_est = get_estimate(ti_kJ)
    TI_est_err = get_estimate(dti_kJ)
...
if estimator.lower() == 'mbar' or estimator.lower() == 'bar' or estimator.lower() == 'all':
   #Obtain u_nk reduced potentials and remove uncorrelated samples
    ...
    if estimator.lower() == 'mbar' or estimator.lower() == 'all':
        ...
        #Convert Units to KJ/mol
        mbar_kJ = to_kJmol(mbar.delta_f_, T=temp)
        dmbar_kJ = to_kJmol(mbar.d_delta_f_, T=temp)
        mbar_all = pd.concat([mbar_kJ, dmbar_kJ])
        mbar_all.to_csv('MBAR.csv')

        #Get estimates from df
        MBAR_est = get_estimate(mbar_kJ)
        MBAR_est_err = get_estimate(dmbar_kJ)
    ...
    if estimator.lower() == 'bar' or estimator.lower() == 'all':
        ...
        bar_kJ = to_kJmol(bar.delta_f_, T=temp)
        dbar_kJ = to_kJmol(bar.d_delta_f_, T=temp)
        bar_all = pd.concat([bar_kJ, dbar_kJ])
        ...
        #Get estimates from df
        BAR_est = get_estimate(bar_kJ)
        BAR_est_err = get_estimate(dbar_kJ)
...
print(errors)
df_est = pd.DataFrame({'Estimator': estimators, 'FE Estimate (kJ/mol)': estimates, 'Error (kJ/mol)': errors})
